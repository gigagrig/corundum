...
int mqnic_scheduler_enable(struct mqnic_sched *sched)
{
	...
	// enable scheduler
	MqnicWriteRegister(1, sched->rb->regs + MQNIC_RB_SCHED_RR_REG_CTRL);

	// enable queues
	for (k = 0; k < sched->channel_count; k++)
		MqnicWriteRegister(3, sched->hw_addr + k * sched->channel_stride);
...
{
	// disable scheduler
	MqnicWriteRegister(0, sched->rb->regs + MQNIC_RB_SCHED_RR_REG_CTRL);
}
